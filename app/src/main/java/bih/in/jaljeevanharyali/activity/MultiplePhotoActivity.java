...
public class MultiplePhotoActivity extends Activity implements OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
       ...
       // Utiilties.setStatusBarColor(this);
        ...
        Cursor cursor = db
                .rawQuery(
                        "SELECT Photo1,Photo2,Photo3,Photo4 FROM PondInspectionDetail where id =?",
            ...
            switch (i) {
                case 0:
                    ...
                    if (imageData1!=null) {
                     ...
                     long c= db.update("PondInspectionDetail", values, "id=?", whereArgs);
                        btnOk.setEnabled(true);
                     if(c>0)
                     {
    ...
    public void onBackPressed() {
        if (purpose.equalsIgnoreCase("new")){
            if (exitCount < 4){
                ...
                long isDel = db.resetPondInspectionUpdatedData(String.valueOf(PID));
                finish();
            }
        }else{
            finish();
