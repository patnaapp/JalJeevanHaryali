...
@SuppressLint("NewApi")
public class ManregaPhotoActivity extends Activity implements OnClickListener {
...
//        consumerBill = getIntent().getStringExtra("consumerBill");
        ...
        Cursor cursor = db
                .rawQuery(
                        "SELECT "+columnPhoto1+","+columnPhoto2+",Photo3,Photo4 FROM Menrega_Rural_Dev_Dept where id =?",
...
//                            btnOk.setBackgroundResource(R.drawable.buttonbackshape);
    ...
    private void saveImage() {
        ...
        int i = 0;
        DataBaseHelper placeData = new DataBaseHelper(ManregaPhotoActivity.this);
            ...
            switch (i) {
                ...
                case 1:
                    if (imageData2!=null) {
                        ...
                        whereArgs = new String[]{String.valueOf(PID)};
                        db.update("Menrega_Rural_Dev_Dept", values, "id=?", whereArgs);
                    }
                    break;
                case 2:
                    if (imageData3!=null) {
                        ...
                        whereArgs = new String[]{String.valueOf(PID)};
                        long c1= db.update("Menrega_Rural_Dev_Dept", values, "id=?", whereArgs);
                        if(c1>0)
                        {
                            Log.e("IMAGEE","Updated");
                        }
    ...
    public void onBackPressed() {
        if (purpose.equalsIgnoreCase("new")){
            if (exitCount < 4){
                ...
                //long isDel = db.deleteManregadData(String.valueOf(PID));
                finish();
            }
        }else{
            finish();
